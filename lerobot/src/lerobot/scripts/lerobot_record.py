...
class RecordConfig:
    ...
    # Display data on a remote Rerun server (optional; local viewer if unset)
    display_ip: str | None = None
    display_port: int | None = None
    # Whether to send compressed images to Rerun (auto when remote ip/port set)
    display_compressed_images: bool = False
    resume: bool = False
    # Audio cues during record / reset (e.g. espeak)
    ...
    try:
        ...
        with VideoEncodingManager(dataset):
            ...
            while recorded_episodes < cfg.dataset.num_episodes and not events["stop_recording"]:
                ...
                n_frames = 0 if dataset.episode_buffer is None else int(dataset.episode_buffer.get("size", 0))
                if n_frames == 0:
                    logging.warning(
                        "Skipping save_episode: no frames recorded this episode (Right Arrow exits the current "
                        "loop early; if pressed before the first frame, nothing is saved). Use Esc to stop all "
                        "episodes. Repeating this episode without incrementing the counter."
                    )
                    continue
